...
int main(int argc, char **argv) {
  ...
  while (true) {
    ...
    while (true) {
      ...
      fprintf(stdout, "Received: begin=%llu end=%llu mod=%llu\n", begin, end, mod);

      pthread_t threads[tnum];
      struct FactorialArgs args[tnum];

      uint64_t chunk_size = (end - begin + 1) / tnum;
      uint64_t chunk_begin = begin;

      for (int i = 0; i < tnum; i++) {
        args[i].begin = chunk_begin;
        args[i].end = (i == tnum - 1) ? end : chunk_begin + chunk_size - 1;
        args[i].mod = mod;
        chunk_begin = args[i].end + 1;
